...
The **Pen buttons** pane in the expanded editor gets around that for the buttons: map button 1, button 2 or the eraser end to a left, middle or right click, or to holding Space, and the background service runs a small helper (`tools/pen-buttons.py`, standard-library Python) that reads the pen's evdev node without taking it away from libinput and presses that button on a virtual mouse (or the key on a virtual keyboard), where the pen already put the cursor. It works in every app, including ones without tablet support. Apps that already use the buttons would get them twice, so the default is "let the app decide". Button 1 is the pen's `BTN_STYLUS` switch, usually the one nearer the tip; button 2 is `BTN_STYLUS2`.

Two things worth knowing before you map. A middle click from the virtual mouse is a middle click, so in browsers it also pastes the primary selection, exactly as a mouse's middle button does; **Hold Space** is the pan gesture of Excalidraw, Krita, GIMP, Inkscape and most other drawing apps without that side effect. And the eraser row only appears when the tablet can report an eraser tool: libwacom knows the pens of most Wacom models, but for a pen it has no data on (it then lists only its generic "General Pen") the Tablet pane shows what the tablet accepts, which can be more than the pen in the box has, so the row is labelled "eraser end, if any".

The virtual devices need `/dev/uinput`; when it is not open to your user, the pane offers a one-time udev rule (`uaccess`, asks for your password).
